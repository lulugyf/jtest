 ...
 * 
 * @author guanyf
 * 从 des4G.c 手工翻译， 只处理了padding_des, 有需要再翻译其它的 
 */
...
//        printArr(src);
        ...
        y <<= 8;
        y |= src[0] & 0xff;
//        System.out.printf("1== x=%08x y=%08x z=%08x\n", x, y, z);
        z  = ((x >>> 004) ^ y) & 0x0F0F0F0F;
        x ^= z << 004;
        y ^= z;
        z  = ((y >>> 020) ^ x) & 0x0000FFFF;
...
//        printArr(dst);
    ...
    public static final int CRYPTO_TFM_REQ_WEAK_KEY = 0x00000100;
    public static final int CRYPTO_TFM_RES_WEAK_KEY = 0x00100000;

    static final int setkey_sc(int[] expkey, byte[] key, int flags)
    {
         int n, w;
         ...
         if ((flags & CRYPTO_TFM_REQ_WEAK_KEY) > 0
             && ((n - (w >>> 3)) & w) != 0) {  /* 1 in 10^10 keys passes this test */
                 if (n < 0x41415151) {
                         if (n < 0x31312121) {
                                 if (n < 0x14141515) {
                                         /* 01 01 01 01 01 01 01 01 */
                                         if (n == 0x11111111) return CRYPTO_TFM_RES_WEAK_KEY;
                                         /* 01 1F 01 1F 01 0E 01 0E */
                                         if (n == 0x13131212) return CRYPTO_TFM_RES_WEAK_KEY;
                                 } else {
                                         /* 01 E0 01 E0 01 F1 01 F1 */
                                         if (n == 0x14141515) return CRYPTO_TFM_RES_WEAK_KEY;
                                         /* 01 FE 01 FE 01 FE 01 FE */
                                         if (n == 0x16161616) return CRYPTO_TFM_RES_WEAK_KEY;
                                 }
                         } else {
                                 if (n < 0x34342525) {
                                         /* 1F 01 1F 01 0E 01 0E 01 */
                                         if (n == 0x31312121) return CRYPTO_TFM_RES_WEAK_KEY;
                                         /* 1F 1F 1F 1F 0E 0E 0E 0E (?) */
                                         if (n == 0x33332222) return CRYPTO_TFM_RES_WEAK_KEY;
                                 } else {
                                         /* 1F E0 1F E0 0E F1 0E F1 */
                                         if (n == 0x34342525) return CRYPTO_TFM_RES_WEAK_KEY;
                                         /* 1F FE 1F FE 0E FE 0E FE */
                                         if (n == 0x36362626) return CRYPTO_TFM_RES_WEAK_KEY;
                                 }
                         }
                 } else {
                         if (n < 0x61616161) {
                                 if (n < 0x44445555) {
                                         ...
                                         if (n == 0x41415151) return CRYPTO_TFM_RES_WEAK_KEY;
                                         /* E0 1F E0 1F F1 0E F1 0E */
                                         if (n == 0x43435252) return CRYPTO_TFM_RES_WEAK_KEY;
                                 } else {
                                         /* E0 E0 E0 E0 F1 F1 F1 F1 (?) */
                                         if (n == 0x44445555) return CRYPTO_TFM_RES_WEAK_KEY;
                                         /* E0 FE E0 FE F1 FE F1 FE */
                                         if (n == 0x46465656) return CRYPTO_TFM_RES_WEAK_KEY;
                                 }
                         } else {
                                 if (n < 0x64646565) {
                                         /* FE 01 FE 01 FE 01 FE 01 */
                                         if (n == 0x61616161) return CRYPTO_TFM_RES_WEAK_KEY;
                                         /* FE 1F FE 1F FE 0E FE 0E */
                                         if (n == 0x63636262) return CRYPTO_TFM_RES_WEAK_KEY;
                                 } else {
                                         /* FE E0 FE E0 FE F1 FE F1 */
                                         if (n == 0x64646565) return CRYPTO_TFM_RES_WEAK_KEY;
                                         /* FE FE FE FE FE FE FE FE */
                                         if (n == 0x66666666) return CRYPTO_TFM_RES_WEAK_KEY;
                                 }
                         }
                 }
         ...
         byte[] k;
         byte[] b0, b1;
         byte[] bits0 = new byte[56];
         byte[] bits1 = new byte[56];
         ...
         do {
             w   = (b1[k[l+ 0   ]] | b0[k[l+ 1   ]]) << 4;
             w  |= (b1[k[l+ 2   ]] | b0[k[l+ 3   ]]) << 2;
             w  |=  b1[k[l+ 4   ]] | b0[k[l+ 5   ]];
             w <<= 8;
             w  |= (b1[k[l+ 6   ]] | b0[k[l+ 7   ]]) << 4;
             w  |= (b1[k[l+ 8   ]] | b0[k[l+ 9   ]]) << 2;
             w  |=  b1[k[l+10   ]] | b0[k[l+11   ]];
             w <<= 8;
             w  |= (b1[k[l+12   ]] | b0[k[l+13   ]]) << 4;
             w  |= (b1[k[l+14   ]] | b0[k[l+15   ]]) << 2;
             w  |=  b1[k[l+16   ]] | b0[k[l+17   ]];
             w <<= 8;
             w  |= (b1[k[l+18   ]] | b0[k[l+19   ]]) << 4;
             w  |= (b1[k[l+20   ]] | b0[k[l+21   ]]) << 2;
             w  |=  b1[k[l+22   ]] | b0[k[l+23   ]];
             expkey[j] = w;
             
             w   = (b1[k[l+ 0+24]] | b0[k[l+ 1+24]]) << 4;
             w  |= (b1[k[l+ 2+24]] | b0[k[l+ 3+24]]) << 2;
             w  |=  b1[k[l+ 4+24]] | b0[k[l+ 5+24]];
             w <<= 8;
             w  |= (b1[k[l+ 6+24]] | b0[k[l+ 7+24]]) << 4;
             w  |= (b1[k[l+ 8+24]] | b0[k[l+ 9+24]]) << 2;
             w  |=  b1[k[l+10+24]] | b0[k[l+11+24]];
             w <<= 8;
             w  |= (b1[k[l+12+24]] | b0[k[l+13+24]]) << 4;
             w  |= (b1[k[l+14+24]] | b0[k[l+15+24]]) << 2;
             w  |=  b1[k[l+16+24]] | b0[k[l+17+24]];
             w <<= 8;
             w  |= (b1[k[l+18+24]] | b0[k[l+19+24]]) << 4;
             w  |= (b1[k[l+20+24]] | b0[k[l+21+24]]) << 2;
             w  |=  b1[k[l+22+24]] | b0[k[l+23+24]];
    ...
    static int des_setkey(des_ctx ctx, byte[] key, int flags)
    {
             return setkey_sc(ctx.expkey, key, flags);
            
    }
     
    static void des_encrypt(des_ctx ctx, byte[] dst, byte[] src)
    {
             des_small_fips_encrypt(ctx.expkey, dst, src);
    }
     
    static void des_decrypt(des_ctx ctx, byte[] dst, byte[] src)
    {
             des_small_fips_decrypt(ctx.expkey, dst, src);
    }
     
    static final String desecb_encrypt(String src, String key)
    {
        byte[] tmpDst = new byte[8];
        byte[] tmp = new byte[8]; 
        des_ctx ctx1 = new des_ctx();
        int i, j, x;
        int flags = 1;
        
        byte[] key1 = key.getBytes(); //fromBCD(key);
        if( key1.length < 8 ) {
        	System.out.println("invalid key, size must be 8.");
        	return null;
        }
        des_setkey(ctx1, key1, flags);
        byte[] srcTmp = src.getBytes(); //fromBCD(src);
        if(srcTmp.length % 8 != 0) {
        	System.out.println("invalid src, size must be 8*n.");
        	return null;
        }
        byte[] encTmp = new byte[srcTmp.length];
        ...
        for(;j<i/8;j++)
        {
        	System.arraycopy(srcTmp, x, tmp, 0, 8);
    	    des_encrypt(ctx1, tmpDst, tmp);
    	    System.arraycopy(tmpDst, 0, encTmp, x, 8);
    	    x += 8;
         }
         return toBCD(encTmp);  
    }
    static final String desecb_decrypt(String src, String key)
    {
        byte[] tmpDst = new byte[8];
        byte[] tmp = new byte[8]; 
        des_ctx ctx1 = new des_ctx();
...
//        printArr(srcTmp);
        ...
        {
        	System.arraycopy(srcTmp, x, tmp, 0, 8);
    	    des_decrypt(ctx1, tmpDst, tmp);
    	    System.arraycopy(tmpDst, 0, encTmp, x, 8);
    	    x += 8;
         }
         return new String(encTmp);  
    }
    
    
    public static String des_ecb_pkcs5padding(String src, String key,int mode)
    {
    	int klen = key.length();
    	if(klen > 8) {
    		key = key.substring(0, 8);
    	}else if(klen < 8) {
    		byte[] p = {0,0,0,0,0,0,0};
    		key = key + new String(p, 0, 8-klen);
    	}
    	//String key16 = toBCD(key.getBytes());

        if (mode == 0)
        {
        	int slen = src.length();
//        	System.out.printf("en slen = %d\n", slen);
        	if(slen % 8 > 0) { /*加密数据补位*/
        		byte[] p = {0,0,0,0,0,0,0};
        		int x = 8 - slen % 8;
//        		System.out.printf("en x = %d\n", x);
        		for(int i=0; i<x; i++) p[i] = (byte)x;
        		src = src + new String(p, 0, x);
        	}
        	return desecb_encrypt(src, key);
         }
        else if (mode == 1)
         {
            String des = desecb_decrypt(src, key);
            //解密去补位
            int dlen = des.length();
            if(des.charAt(dlen - 1) < 8) {
            	int x = des.charAt(dlen-1);
            	des = des.substring(0, dlen-x);
...
//        des_small_fips_decrypt(expkey, dst, src);
        System.out.println(des_ecb_pkcs5padding("1234567890", "NETTV2015", 0));
        System.out.println(des_ecb_pkcs5padding("915772B99F4127D54422BA2308667D94", "NETTV2015", 1));
        
//        printArr(fromBCD("915772B99F4127D5"));
    }
}
